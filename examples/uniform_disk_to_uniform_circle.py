...
def main():
    ...
    plot_mtransport_validation_2d(res, mu0, mu1, alpha=alpha, nsim=256, key=ksim, bins=32, usetex=True, save_prefix="./examples/uniform_disk_to_uniform_circle", show=False, nticks=2, ax_lims=ax_lims)
    
    # simulate Bass martingale and create animations
    key, ksim = jax.random.split(key)
    M = res.sim_bass_martingale(
        x=mu0[:512],
        nsteps=256,
        ncond=512,
        jit_compile=True,
