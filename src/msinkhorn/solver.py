...
class NMOTPotentials():
    ...
    def to_mu0(self, a: jnp.ndarray, key: jax.random.PRNGKey, nsim: int) -> jnp.ndarray:
        """Push samples from alpha to samples from mu0 using Monte Carlo convolution."""
        key, sub = jax.random.split(key)
        z = jax.random.normal(sub, (nsim, *a.shape))
        b = jnp.atleast_2d(a)[None, :, :] + z
    ...
    def mtransport(self, x: jnp.ndarray, z: jnp.ndarray) -> jnp.ndarray:
        ...
        Args:
            x (jnp.ndarray): Samples from mu0, shape [batch_size, d].
            z (jnp.ndarray): normally distributed random variables, shape [batch_size, d]. Represent the Brownian noise added to alpha to get alpha_conv.

        Returns:
            y (jnp.ndarray): Samples from mu1, shape [batch_size, d].
        """
        x_2d = jnp.atleast_2d(x)
        z_2d = jnp.atleast_2d(z)
        assert x_2d.shape == z_2d.shape, f"Shapes of x {x_2d.shape} and z {z_2d.shape} are incompatible."
        return self.to_mu1(self.to_alpha(x_2d) + z_2d)
    
    def sim_bass_martingale(self, key: jax.random.PRNGKey, x: jnp.ndarray, nsteps: int, ncond: int, batch_size: Optional[int] = None, jit_compile: Optional[bool] = False) -> jnp.ndarray:
        ...
        Args:
            ...
            jit_compile (Optional[bool]): Whether to JIT compile the simulation function.
        Returns:
        ...
        if batch_size is None:
            batch_size = npaths
        niter = (npaths + batch_size - 1) // batch_size
        keys = jax.random.split(key, niter)
        dtype = x.dtype

        def _sim_bass_martingale_batch(key: jax.random.PRNGKey, x: jnp.ndarray) -> jnp.ndarray:
            # simulate Brownian motion paths
            n = x.shape[0]
            alpha = self.to_alpha(x)                                # [n, d]
        
            key_bm, key_y = jax.random.split(key, 2)
            dt = 1.0 / (nsteps - 1)
            dW_t = jnp.sqrt(dt) * jax.random.normal(key_bm, (n, nsteps - 1, d), dtype=dtype)
            incr = jnp.concatenate([jnp.zeros((n, 1, d), dtype=dtype), jnp.cumsum(dW_t, axis=1)], axis=1)
            B_t = alpha[:, None, :] + incr                          # [n, nsteps, d]

            # simulate ncond B_1 given B_t
            t = jnp.linspace(0.0, 1.0, nsteps)
            sigma = jnp.sqrt(1.0 - t)
            sigma = sigma.at[-1].set(0.0)
            z = jax.random.normal(key_y, (n, nsteps, ncond, d), dtype=dtype)
            B_t = B_t[:, :, None, :]
            B_1 = B_t + sigma[None, :, None, None] * z

            # push B_1 to mu1
            y = self.to_mu1(B_1.reshape(-1, d))                     # [n * nsteps * ncond, d]
            return y.reshape(n, nsteps, ncond, d).mean(axis=2)      # [n, nsteps, d]
        
        if jit_compile:
            pad = niter * batch_size - npaths
            pad_cfg = ((0, pad),) + tuple((0, 0) for _ in range(x.ndim - 1))
            x_pad = jnp.pad(x, pad_cfg) if pad > 0 else x
            x_batches = x_pad.reshape((niter, batch_size) + x.shape[1:])

            fwd = jax.jit(_sim_bass_martingale_batch)
            out = [fwd(keys[i], x_batches[i]) for i in range(niter)]
            M = jnp.concatenate(out, axis=0)[:npaths]
        else:
            batch = [slice(batch_size * i, min(batch_size * (i + 1), npaths)) for i in range(niter)]
            out = [_sim_bass_martingale_batch(keys[i], x[batch[i]]) for i in range(niter)]
            M = jnp.concatenate(out, axis=0)
        return M
